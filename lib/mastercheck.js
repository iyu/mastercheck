        ...
        for (i = 0; i < master.length; i++) {
            if (format[0] instanceof formatTypes.Number ||
                ...
                if (!format[0].check(master[i])) {
                    errs.push({ key: i, value: master[i] });
                }
                ...
                for (j = 0; j < _errs.length; j++) {
                    errs.push({ key: i + '.' + _errs[j].key, value: _errs[j].value });
                }
                ...
                errs.push({ key: i, value: JSON.stringify(master[i]) });
            }
        }
    } else {
        for (key in format) {
            if (format[key] instanceof formatTypes.Number ||
                ...
                if (!format[key].check(master[key])) {
                    errs.push({ key: key, value: master[key] });
                }
                ...
                errs.push({ key: key, value: JSON.stringify(master[key]) });
            }
        }
    }
